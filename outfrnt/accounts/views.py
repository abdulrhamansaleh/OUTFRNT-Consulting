    ...
    if 'join' in request.POST:
        if form.is_valid():
            ...
            send_mail(
                ...
                # email body 
                f'Name: {first} {last}\nPhone: {phone}\nEmail: {email}\nInquiry: {help_body}',
                # email sender (PERSON SENDING INQUIRY)
                f'{email}',
                # email receiver (OUTFRNT)
                ['pureexec@gmail.com'],# Deployment 'noreply@outfrnt.com'
                fail_silently = False
            )
            # notify user that their inquiry has been received
            
            # email content
            subject = "Let's get your business OUTFRNT"
            body = f"Welcome {first}\n"\
